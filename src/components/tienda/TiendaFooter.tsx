...
export function TiendaFooter({ tienda }: TiendaFooterProps) {
  ...
  return (
    <footer className="bg-[var(--secondary)] border-t border-[var(--muted)]/30 mt-12 sm:mt-16 safe-area-inset-bottom">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-8 md:py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 sm:gap-6 md:gap-8">
          <div>
            <h3 className="text-base sm:text-lg font-semibold text-[var(--text)] mb-2 sm:mb-3">{tienda.nombre}</h3>
            <p className="text-[var(--muted)] text-sm">
              Gracias por visitarnos. Contáctanos por WhatsApp para cualquier consulta.
            </p>
          </div>
          <div>
            <h3 className="text-base sm:text-lg font-semibold text-[var(--text)] mb-2 sm:mb-3">Contacto</h3>
            <a
              href={whatsappLink}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 min-h-[44px] text-[var(--muted)] hover:text-[var(--primary)] active:opacity-80 transition-colors touch-manipulation"
            >
